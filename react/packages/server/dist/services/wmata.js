 ...
 */
export function getArrivalAtStation(entities, tripId, destinationCode) {
    const now = Date.now() / 1000;
 ...
 */
export function enrichTrainsWithDestinationArrival(trains, entities, destinationCode) {
 ...
 */
export async function fetchDestinationArrivals(originTrains, destinationCode, apiKey, gtfsEntities) {
    ...
    return originTrains.map(train => {
        ...
        const matchingTrains = destPredictions.filter(destTrain => {
            if (destTrain.Line !== train.Line)
                return false;
            // Must have same destination name (same direction)
            if (normalizeDestination(destTrain.DestinationName) !== normalizeDestination(train.DestinationName))
                return false;
