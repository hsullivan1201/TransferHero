 ...
 */
export function findAllPossibleTransfers(fromCode, toCode, transferWalkTime = DEFAULT_TRANSFER_WALK_TIME) {
 ...
 */
export function evaluateTransferRoute(fromCode, toCode, transfer, transferWalkTime = DEFAULT_TRANSFER_WALK_TIME) {
 ...
 */
export function findTransfer(fromCode, toCode, transferWalkTime = DEFAULT_TRANSFER_WALK_TIME) {
    const fromStation = findStationByCode(fromCode);
    const toStation = findStationByCode(toCode);
    if (!fromStation || !toStation)
        return null;
    ...
    const evaluatedRoutes = allTransfers.map(transfer => evaluateTransferRoute(fromCode, toCode, transfer, transferWalkTime));
