 ...
 */
export const PLATFORM_CODES: Record<string, Partial<Record<Line, string>>> = {
 ...
 */
export function normalizePlatformCode(code: string, availableStations: string[]): string {
 ...
 */
export function getAllPlatformCodes(code: string): string[] {
  const alias = STATION_ALIASES[code]
 ...
 */
export function getPlatformForLine(stationCode: string, line: Line): string {
