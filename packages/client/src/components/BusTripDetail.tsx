...
import { Bus, Footprints, RefreshCw, ExternalLink, Rss, Loader2, Clock3, Train as TrainIcon } from 'lucide-react'
...
export function BusTripDetail({
  ...
  const metroWait = deriveWaitMinutes(displayTrain, departureTimestamp)
    ?? (metroTrains.length > 0 ? Math.max(0, getTrainMinutes(metroTrains[0].Min)) : null)
  ...
  // For direct trips: derive ride time from first train's _destArrivalMin
  const directRide = useMemo((): number | null => {
    if (!isDirect) return null
    const train = displayTrain ?? metroTrains[0]
    if (!train) return null
    if (train._destArrivalMin != null && metroWait != null) {
      return Math.max(0, Math.round(train._destArrivalMin - metroWait))
    }
    return trip.metroTimeMinutes // fallback to estimate
  }, [isDirect, displayTrain, metroTrains, metroWait, trip.metroTimeMinutes])
  ...
  }, [busPredictions, isMetroBus, estimatedArrivalAtBusStop, busLeg.boardWalkMinutes])
      ...
      {isMetroBus && (
        <>
          <BusWalkCard
            ...
            searchQuery={toName + ' Metro Station'}
          />
...
function BusLegPanel({ busLeg, isFirst, arrivalAtBusStopMin, predictions, predictionsLoading }: {
  ...
  // Scheduled departures that come after all RT predictions (fill the gap for
  // buses that haven't started their trip yet and thus have no RT data).
  // Includes wait time relative to estimated arrival at bus stop.
  const scheduledAfterRT = useMemo(() => {
    const scheduled = busLeg.scheduledDepartures
    if (!scheduled || scheduled.length === 0) return []
    // Find the latest RT prediction time to avoid duplicates
    const lastRtMin = predictions.length > 0
      ? Math.max(...predictions.map(p => p.minutes))
      : -1
    // Only show scheduled departures past the last RT prediction
    return scheduled
      .filter(s => s.minutesFromNow > lastRtMin + 2) // 2 min buffer to avoid near-duplicates
      .slice(0, 2)
      .map(s => ({
        ...s,
  ...
  }, [busLeg.scheduledDepartures, predictions, arrivalAtBusStopMin])
        ...
        {predictionsLoading ? (
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              {/* Scheduled departures (after RT) */}
              {scheduledAfterRT.map((s, i) => (
                <div
                  key={`sched-${i}`}
                  className="flex items-center justify-between p-3 rounded-lg bg-[var(--bg-secondary)] border border-[var(--border-color)] opacity-75"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-[var(--text-secondary)]">
                        {s.waitTime != null ? `${s.waitTime} min wait · ` : ''}Scheduled
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-xl font-bold text-[var(--text-primary)]">
                      {s.minutesFromNow} min
                    </div>
                    <div className="text-xs text-[var(--text-secondary)]">
                      {s.departureTime}
                    </div>
